...
sub get_genome_objects {
	...
	elsif ($_ =~ /^p3\|(\d+\.\d+)$/)       # PATRIC genome
	{
	    my $gid = $1;
	    get_patric_genome_object($gtoD, $gid, \*TYPES);
	}
...
sub get_patric_genome_object
{
    ...
    if (!$gto)
    {
	warn "Could not retrieve $gid\n";
	return;
    }
    $gto->destroy_to_file("$gtoD/$gid");
    print $types_fh join("\t", $gid, 'PATRIC'),"\n";
}
...
to the family described by the row.  The sixth column contains just the row
